...
void parse_args(int argc, char *argv[]){
    ...
    else if ( string(argv[1]).compare(string("-gr")) == 0 )
    {
        if( argc != 6 ){
            print_usage();
        }
        srand(time(NULL));
        ELEMENTNUM = atoi(argv[2]);
        AVG_COVER_TIMES = atoi(argv[3]);
        cout << "generating data..." << endl;
        printf("ELEMENTNUM = %d\nAVG_COVER_TIMES = %d\npath = %s\n", ELEMENTNUM, AVG_COVER_TIMES, argv[5]);
        generate_realworld(argv[4], argv[5]);
    }
    ...
    else if ( string(argv[1]).compare(string("-r")) == 0 )
    {
        ...
        if( itemnum == 0 )
        {
            printf("%s: %lld\n", argv[2], current_uncovered_weight);
            return ;
        }
        settings();
	    srand(seed); 
        ...
        // fprintf(fp, "%-50s\t%-15lld\t%.2f\n", argv[2], best_uncovered_weight, opt_time);
        // fclose(fp);
    ...
    else if ( string(argv[1]).compare(string("-cp")) == 0 )
    {
        if( argc != 4 )
        {
            print_usage();
        }
        cout << "changing to cplex" << endl;
        change_to_cplex(argv[2], argv[3]);
    }
